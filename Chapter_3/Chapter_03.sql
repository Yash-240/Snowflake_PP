...
--im using aws details here for simplicity - not using azure as per book 
--below code will create a "storage integration object" for AWS S3 in snowflake. 
create storage integration BISTRO_INTEGRATION
  type = external_stage
  storage_provider = 'S3'
  enabled = true
  STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::311141564977:role/snowflakeuserrole'
  STORAGE_ALLOWED_LOCATIONS = ('*')
  --STORAGE_BLOCKED_LOCATIONS = 's3://bakeryorders007/';
...
-- the below is only for Microsoft azure - direct connection to blob storage using a SAS token
-- you can skip this if you are using AWS S3 
